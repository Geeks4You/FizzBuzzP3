...
void CFizzbuzz::putFizzbuzz() {
	cout << "Player Name: " << this->pname << endl;

	if ((this->itemNumber % 3 == 0) && (itemNumber % 5 == 0)) {
		cout << "Item Number: " << itemNumber << " - FizzBuzz" << endl;
	}
	else if (itemNumber % 3 == 0) {

		cout << "Item Number: " << itemNumber << " - Fizz" << endl;
	}
	else if (itemNumber % 5 == 0) {

		cout << "Item Number: " << itemNumber << " - Buzz" << endl;
	}
}
// getRecord() function so the program can detect when a zero item number is entered
int CFizzbuzz::getRecord() {
	if (this->itemNumber == 0) {
		cout << "Game is over because the item number zero was entered" << endl;
		return 0;
	}
	return itemNumber;
}
